...
Let's find the TV Episode with the perfect rating and 200,000 IMDb ratings.First, we need the information from the 'title_ratings' and 'title_basics'. Then, we will find the TV episode with 10/10 rating and more 200,000 votes.
...
ggplot(top_genre_each_decade, aes(x = decade, y = success_count, fill = genres)) +
    geom_col(position = "dodge") +
    labs(title = "Top Genres by Success in Each Decade",
         x = "Decade",
         y = "Success Count") +
    theme_minimal() + 
...
ggplot(successful_actors, aes(x = reorder(primaryName, success_count), y = success_count, fill = primaryName)) +
    geom_bar(stat = "identity") +
    labs(title = "Top Successful Actors in Action Movies",
         x = "Actors",
         y = "Success Count") +
    theme_minimal() +
